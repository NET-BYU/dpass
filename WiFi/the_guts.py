...
from decoders import dynamic_decoder, Interval_Decoder, dynamic_decoder2
...
class Guts:
    def __init__(self, wireless_interface, interval, frequency, chip_sample_number):
        ...
        self.std_decoder = dynamic_decoder2(samples_per_chip=chip_sample_number)
        self.method_functions = {"auto_correlation": self.auto_correlation, "interval":self.interval_function, "std_auto":self.std_auto_function}
        self.colors = {
            "0": "red",
            "1": "orange",
            "2": "green",
                    ...
                    if self.interval - 10 < cum_time:  # we have hit a interval
                        ...
                        while (
                            ...
                            if test is not None:  # we found a symbol
                                print(test)
                                # print(test)
                                symbols_caught.append(test[0])
                                if len(symbols_caught) > 8: # full buffer and ready to check for known symbol
                                    symbols_caught.pop(0)
                                    if switch and symbols_caught == ["-4", "-4", "0", "2","4","3","-1","0"]:
                                        print("protocol enacted, switching frequencies")
    ...
    def pass_decode(self, sample_file, method, graph, count):
        print(
                ...
                for tsf, freq, noise, rssi, pwrs in SpectrumFileReader.decode(
                    ...
                    if self.interval - 10 < cum_time:  # we have hit a interval
                        ...
                        while (
                            ...
                            if test is not None and graph:  # we found a symbol
                                location.append(
                                    [
                                        (
                                            len(measurements)
    ...
    def std_auto_function(self,sample):
        return self.std_decoder.add_val(sample)
