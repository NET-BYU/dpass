...
class dynamic_decoder2:
    def __init__(self, samples_per_chip=1):
        ...
        self.standard_deviation = 2
        self.mean_corr = 4
        self.buff_len = 800 * len(self.message_array) * self.max_len

        # self.buff_len = 100 * len(self.message_array) * self.max_len
    ...
    def add_val(self, new_data):
        ...
        if len(self.data_array) > self.max_len:
            ...
            corr_data = []
            for msg in self.message_array:
                x = np.correlate(msg - np.ones(self.max_len), self.transformed_data)[0]
                corr_data.append(x)
                self.mean_corr = (x + self.mean_corr * (self.buff_len - 1)) / (self.buff_len)
                self.standard_deviation = (abs(x - self.mean_corr) + self.standard_deviation * (self.buff_len - 1)) / (self.buff_len)
            curr_max = np.argmax(np.abs(corr_data))
            if np.abs(corr_data[curr_max]) > np.abs(self.the_symbol[1]):
                if corr_data[curr_max] > 0:
                    is_pos = True
                    self.the_symbol = [str(curr_max), corr_data[curr_max], self.count]
                else:
                    is_pos = False
                    self.the_symbol = ["-"+str(curr_max), corr_data[curr_max], self.count]
            if self.count < 2250:
