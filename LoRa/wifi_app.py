...
@click.option(
    "-m",
    "--method",
    type=click.Choice(["auto_correlation", "interval"]),
...
@click.option(
    "-m", "method", type=click.Choice(["auto_correlation", "interval"]), default="auto_correlation"
)
@click.option("-g", "--graph", is_flag=True)
def passive_collection_decode(ctx, sample_file, method, graph):
    guts = ctx.obj["guts"]
    guts.pass_decode(sample_file, method, graph)
