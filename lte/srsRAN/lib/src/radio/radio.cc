...
{
  // Create a new publishing socket
  zmq_ctx  = zmq_ctx_new();
...
// Transmits RSSI values to zmq socket
int radio::tx_zmq(cf_t* buff, uint32_t buff_length)
{
  // Make a message zmq socket
  zmq_msg_t envelope;
  const int rmi = zmq_msg_init_size(&envelope, sizeof(int16_t));

  // Check that the socket is already empty
  uint8_t dummy;
  zmq_recv(zmq_sock, &dummy, sizeof(dummy), 0);

  // Will change the IQ values to be RSSI values and cast them to the correct type for the socket
  rssi_buff[0] = (int16_t)srsran_convert_power_to_dB(srsran_vec_avg_power_cf(buff, buff_length));
  memcpy((void*)((char*)zmq_msg_data(&envelope)), rssi_buff, sizeof(int16_t));

  // Sends the RSSI values to our socket
  const size_t rs = zmq_msg_send(&envelope, zmq_sock, 0);

  if (rs != sizeof(int16_t)) {
    printf("RS Value: %ld\n", rs);
...
bool radio::rx_dev(const uint32_t& device_idx, const rf_buffer_interface& buffer, srsran_timestamp_t* rxd_time)
{
  ...
  // Send the values from the buffer to the socket before it passes through the automatic gain controller
  tx_zmq((cf_t*)radio_buffers[0], nof_samples);
...
bool radio::open_dev(const uint32_t& device_idx,
                     FILE**          rx_files,
                     FILE**          tx_files,
...
void radio::set_rx_freq(const uint32_t& carrier_idx, const double& freq)
{
  ...
  if (rx_channel_mapping.allocate_freq(carrier_idx, freq)) {
    ...
    if (device_mapping.channel_idx >= nof_channels_x_dev) {
      logger.error("Invalid mapping physical channel %d to logical carrier %d on f_rx=%.1f MHz (nof_channels_x_dev=%d, "
                   "device_idx=%d)",
